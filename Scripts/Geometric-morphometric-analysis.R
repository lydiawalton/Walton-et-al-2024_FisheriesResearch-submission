################################################################
# Title: Otolith shape analyses (PCA, CVA, disparity)
# Author: Lydia Walton
...
#Sub out species that are not barcode compliant or have less than 3 samples
twoD.sub <- twoD_coords_myct[-c(52,68,79,94,95,96,101,103,104,105,144,146), ]

#twoD.sub <- twoD_coords_myct[!(row.names(twoD_coords_myct) %in% c("Specimen144","Specimen79", "Specimen68", "Specimen 94", "Specimen 96")),]
...
myct.sub <- coords_myct_wSpecies[-c(52,68,79,94,95,96,101,103,104,105,144,146), ]
...
Figure_PCA <- autoplot(myct.pca,data=myct.sub,colour="Species", shape = "Species", frame = TRUE)+
  ...
  scale_shape_manual(values=c(15,16,17,18,20,6,8,0,1,13,9,12,10,5), 
                     breaks = c("Arctozenus risso","Bathylagus pacificus","Chauliodus macouni",
                                ...
                                "Lycodapus mandibularis", "Nansenia candida", 
                                "Stenobrachius leucopsarus", "Stenobrachius nannochir", "Symbolophorus californiensis",
                                "Tactostoma macropus", "Tarletonbeania crenularis" )) +
...
Species_list<-list(
  ...
  L.fier=which(disparity_data_df$species == "Lycodapus mandibularis"),
  N.can=which(disparity_data_df$species == "Nansenia candida"),
  L.spp=which(disparity_data_df$species == "Lycodapus mandibularis"),
  N.spp=which(disparity_data_df$species == "Nansenia candida"),
...
CVA_hull$Species <- factor(CVA_hull$Species, 
                           levels = c("Arctozenus risso","Bathylagus pacificus","Chauliodus macouni",
                                      ...
                                      "Stenobrachius nannochir", "Symbolophorus californiensis",
                                      "Tactostoma macropus","Tarletonbeania crenularis"))
CVAscores_myct$Species <- factor(CVAscores_myct$Species, 
                                 levels = c("Arctozenus risso","Bathylagus pacificus",
                                            "Chauliodus macouni","Diaphus theta","Lampanyctus regalis",
                                            "Lampanyctus ritteri", "Lycodapus mandibularis", "Nansenia candida", 
                                             "Stenobrachius leucopsarus", 
                                            "Stenobrachius nannochir", "Symbolophorus californiensis",
                                            "Tactostoma macropus","Tarletonbeania crenularis"))

CVA_hull$Species <- factor(CVA_hull$Species, levels = c("Arctozenus risso","Bathylagus pacificus",
                                                        "Chauliodus macouni","Diaphus theta","Lampanyctus regalis",
                                                        "Lampanyctus ritteri", "Lycodapus mandibularis", "Nansenia candida", 
                                                        "Stenobrachius leucopsarus", 
                                                        "Stenobrachius nannochir", "Symbolophorus californiensis",
                                                        "Tactostoma macropus","Tarletonbeania crenularis"))
CVAscores_myct$Species <- factor(CVAscores_myct$Species, levels = c("Arctozenus risso","Bathylagus pacificus",
                                                                    "Chauliodus macouni","Diaphus theta","Lampanyctus regalis",
                                                                    "Lampanyctus ritteri", "Lycodapus mandibularis", "Nansenia candida", 
                                                                    "Stenobrachius leucopsarus", 
                                                                    "Stenobrachius nannochir", "Symbolophorus californiensis",
...
myct_cva_plot <- ggplot(CVAscores_myct, aes(x = CV1, y = CV2)) + labs(x="CV1", y="CV2", title="", colour="Species", size=2)+
  ...
  scale_color_viridis(option = "turbo", 
                      ...
                      breaks = c("Arctozenus risso","Bathylagus pacificus","Chauliodus macouni",
                                 "Diaphus theta","Lampanyctus regalis", "Lampanyctus ritteri",
                                 "Lycodapus mandibularis", "Nansenia candida",  
                                 "Stenobrachius leucopsarus", "Stenobrachius nannochir", 
                                 "Symbolophorus californiensis","Tactostoma macropus",
